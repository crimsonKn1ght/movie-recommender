...
This is the "main logic" of the project and is deliberately kept independent of
any web framework. It implements three recommendation algorithms over the
...
The backend imports :class:`AdvancedMovieRecommender` and exposes its results
over HTTP; nothing in here knows about FastAPI.
"""
...
from .data import load_ratings_and_movies

# Neural Collaborative Filtering is optional: TensorFlow is a heavy dependency
# and is not required for the SVD/NMF paths.
try:
    from tensorflow.keras.layers import (
        Concatenate, Dense, Dropout, Embedding, Flatten, Input,
    )
...
class AdvancedMovieRecommender:
    ...
    def load_data(self):
        """Load the MovieLens dataset (downloads on first use)."""
        self.ratings, self.movies = load_ratings_and_movies(self.data_dir)
